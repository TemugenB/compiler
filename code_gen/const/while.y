...
declaration:
	...
	symbol_table[*$2] = var_data( get_line_number(), integer, new_label(), 0 );
        delete $2;
    }
|
	...
	symbol_table[*$2] = var_data( get_line_number(), boolean, new_label(), 0 );
        delete $2;
    }
|
	...
	if( symbol_table.count(*$3) > 0 )
        ...
        // 1 = Const Variable (Unassigned)
        symbol_table[*$3] = var_data( get_line_number(), integer, new_label(), 1 );
        delete $3;
...
|
    T_CONST T_BOOLEAN T_ID T_SEMICOLON
    {
        ...
        // 1 = Const Variable (Unassigned)
        symbol_table[*$3] = var_data( get_line_number(), boolean, new_label(), 1 );
        delete $3;
    }
;
...
assignment:
    T_ID T_ASSIGN expression T_SEMICOLON
    {
        ...
        {
           std::stringstream ss;
           ss << get_line_number() << ": Type error." << std::endl;
           error( ss.str().c_str() );
        }
        /* ----- Solution Start ----- */

        // If state is 2, it is a const that has already been touched.
        if( symbol_table[*$1].const_state == 2 )
        {
           std::stringstream ss;
           ss << get_line_number() << ": Assignment error. '" << *$1 << "' is const and already assigned." << std::endl;
           error( ss.str().c_str() );
        }

        // If state is 1 (Unassigned Const), bump it to 2 (Assigned Const).
        // If state is 0 (Regular), we leave it as 0.
        if( symbol_table[*$1].const_state == 1 )
        {
            symbol_table[*$1].const_state = 2;
        }
