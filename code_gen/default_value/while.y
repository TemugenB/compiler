...
/* ----- Solution: Add declarations and declaration here ----- */
%type <code> declarations declaration
...
program:
	...
	/* ----- Solution: Print initialization code ----- */
        std::cout << *$3 << std::endl; 
        /* --------------------------------------------- */
        std::cout << *$5 << std::endl;
...
declaration:
    T_INTEGER T_ID T_SEMICOLON
    {
        ...
        symbol_table[*$2] = var_data( get_line_number(), integer, new_label() );
	/* ----- Solution ----- */
        $$ = new std::string(""); // No code for uninitialized vars
        /* -------------------- */
        delete $2;
    }
|
    T_BOOLEAN T_ID T_SEMICOLON
    {
        ...
        symbol_table[*$2] = var_data( get_line_number(), boolean, new_label() );
	/* ----- Solution ----- */
        $$ = new std::string(""); 
        /* -------------------- */
        delete $2;
...
|
	...
	/* ----- Solution: Integer with Default Value ----- */
        ...
        {
            std::stringstream ss;
            ss << get_line_number() << ": Type error. Expected integer." << std::endl;
            error( ss.str().c_str() );
        }

        // 4. Generate Code
        // Calculate expression -> mov result into variable memory
        $$ = new std::string(
            $4->expr_code + 
...
|
    ...
    {
    	{
        /* ----- Solution: Boolean with Default Value ----- */
        ...
        {
            std::stringstream ss;
            ss << get_line_number() << ": Type error. Expected boolean." << std::endl;
            error( ss.str().c_str() );
        }

        // Note: boolean uses 'al' register
        $$ = new std::string(
            $4->expr_code + 
